...
if (isset($_POST['save_treatment'])) {
    ...
    echo "treatement_id $id|$type \n";

    if (empty($id) && $type != "package") {
        // Updated column name to duration_minutes
        echo "PUMASOK DITO ANG PACKAGE $type|$duration";
        ...
        // Updated column name to duration_minutes
        $stmt = $pdo->prepare("UPDATE packages SET name=?, description=?, duration_minutes=?, price=?, type=? WHERE package_id=?");
        $stmt->execute([$name, $description, $duration, $price, $type, $id]);
    ...
    // Handle package items logic... error no package_id detected
    ...
    //     }
    // }
    if ($type === 'package' && empty($id)) {
        $stmt = $pdo->prepare("INSERT INTO packages (name, description, price, duration_minutes, type) VALUES (?,?, ?, ?,?)");
        $stmt->execute([$name, $description, $price, $duration, $type]);
        $package_id = $pdo->lastInsertId();

        foreach ($selected_treatments as $t_id) {
            $pdo->prepare("INSERT INTO package_items (package_id, treatment_id) VALUES (?, ?)")
                ->execute([$package_id, $t_id]);
        }
    }
...
$packages = $pdo->query("SELECT * FROM packages WHERE status = 'available' AND type = 'package' ORDER BY name ASC")->fetchAll();

// Logic to get package contents for editing via AJAX or JSON
$package_map = [];

$content_query = $pdo->query("
    SELECT package_id, treatment_id 
    FROM package_items
");
...
<body>
                ...
                <tbody>
                    <?php foreach ($packages as $p): ?>
                        <tr>
                            ...
                            <td class="text-end">
                                <button class="btn btn-sm btn-dark" onclick='editService(<?= json_encode($p) ?>, <?= json_encode($package_map[$p['package_id']] ?? []) ?>)'><i class="bi bi-pencil"></i></button>
                                <form action="" method="POST" class="d-inline">
                                    <input type="hidden" name="archive_id" value="<?= $p['package_id'] ?>">
                                    <button type="submit" class="btn btn-sm btn-dark text-danger"><i class="bi bi-trash"></i></button>
                                </form>
                            </td>
                        </tr>
                                        ...
                                        <div class="form-check custom-check mb-2">
                                            ...
                                            <label class="form-check-label small text-white-50" for="treat_<?= $indiv['treatment_id'] ?>">
                                                ...
                                                <span class="text-gold">
                                                    (<?= isset($indiv['duration_minutes']) ? $indiv['duration_minutes'] : '0' ?>m)
                                                </span>
                                            </label>
                                        </div>
                                    <?php endforeach; ?>
    ...
    <script>
        ...
        function editService(data, contents = []) {
            ...
            // Clear all treatment checkboxes first
            document.querySelectorAll('#packageSelectionArea input[type="checkbox"]')
                .forEach(cb => cb.checked = false);

            document.getElementById('treatment_id').value = data.package_id;
        ...
        document.getElementById('serviceForm').onsubmit = function(e) {
            ...
            if (type === 'package') {
                ...
                if (checked.length < 2) {
                    Swal.fire('Invalid Package', 'A package must include at least 2 different treatments.', 'warning');
                    e.preventDefault();
                    return false;
                }
            }
