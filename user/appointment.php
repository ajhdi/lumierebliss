...
// 1. Get Treatments - We select all and filter manually to avoid the 'status' error
...
<div class="container py-5">
    ...
    if (!empty($treatments)) {
        foreach($treatments as $t): ?>
            <option value="<?= $t['treatment_id'] ?>" <?= $sel_tid == $t['treatment_id'] ? 'selected' : '0' ?>>
        ...
        <?php endforeach; 
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="therapist_id" class="form-select border-0 bg-light py-3" required>
                                    <option value="" disabled <?= empty($sel_thid) ? 'selected' : '' ?>>Choose expert...</option>
                                    <?php foreach($therapists as $th): ?>
                                        <option value="<?= $th['therapist_id'] ?>" <?= $sel_thid == $th['therapist_id'] ? 'selected' : '' ?>>
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="room_id" class="form-select border-0 bg-light py-3" required>
                                    <option value="" disabled <?= empty($sel_rid) ? 'selected' : '' ?>>Select suite...</option>
                                    <?php foreach($rooms as $r): ?>
                                        <option value="<?= $r['room_id'] ?>" <?= $sel_rid == $r['room_id'] ? 'selected' : '' ?>>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="date" name="appointment_date" class="form-control border-0 bg-light py-3" min="<?= date('Y-m-d') ?>" required>
                            </div>
                            <div class="col-md-6">
                                <label class="small fw-bold text-muted text-uppercase">Time</label>
                                <input type="time" name="appointment_time" class="form-control border-0 bg-light py-3" required>
                            </div>
